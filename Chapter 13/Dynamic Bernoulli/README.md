...
This folder models the annual Boat Race outcome as a Bernoulli variable. Its probability evolves through a score-driven recursion: the previous probability and the Bernoulli score residual update the probability for the following year. The default specification is an AR(1) dynamic Bernoulli model; a simpler Markov alternative is retained as an optional setting.
...
From this folder, run MATLAB with `matlab -batch "run('Matlab/BernoulliSetUp.m')"`; R with `Rscript R/DynamicBernoulliModel.R`; or Python with `pip install -r Python/requirements.txt` followed by `python Python/dynamic_bernoulli_model.py`.
